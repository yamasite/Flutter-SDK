...
class RtcChannel
  ...
  /// **Note**
  ///  - 该参数没有默认值，请确保对参数设值。
  ///  - 请勿将该参数设为空字符 ""，否则 SDK 会返回 [ErrorCode.Refused](-5)。
  ///
  /// **Returns**
  /// - 方法调用成功，返回 [RtcChannel] 对象。
  /// - 方法调用失败，返回 null。
  ///  - 如果将 `channelId` 设为空字符 ""，则返回错误码 [ErrorCode.Refused](-5)。
  ...
  /// - 远端：[RtcChannelEventHandler.userJoined] 或 [RtcChannelEventHandler.userOffline]([UserOfflineReason.BecomeAudience])。
  ///
  ...
  /// - SDK 在 [RtcChannelEventHandler.connectionStateChanged] 回调中报告 [ConnectionChangedReason.TokenExpired](9) 错误。
  ///
  ...
  /// 将本地音视频流发布到本频道。
  ///
  /// 该方法的调用需满足以下要求，否则 SDK 会返回 [ErrorCode.Refused](-5)：
  ...
  /// 否则 SDK 会返回 [ErrorCode.Refused](-5)。
...
mixin RtcPublishStreamInterface {
  ...
  ///
  /// Param [transcodingEnabled] 是否转码。如果设为 `true`，则需要在该方法前先调用 [RtcChannel.setLiveTranscoding] 方法。
...
mixin RtcMediaRelayInterface {
  ...
  ///
  /// - 如果 [RtcChannelEventHandler.channelMediaRelayStateChanged] 回调报告 [ChannelMediaRelayState.Running](2) 和 [ChannelMediaRelayError.None](0)，
  /// 且 [RtcChannelEventHandler.channelMediaRelayEvent] 回调报告 [ChannelMediaRelayEvent.SentToDestinationChannel](4)，则表示 SDK 开始在源频道和目标频道之间转发媒体流。
  /// - 如果 [RtcChannelEventHandler.channelMediaRelayStateChanged] 回调报告 [ChannelMediaRelayState.Failure](3)，则表示跨频道媒体流转发出现异常。
  ///
  /// **Note**
...
mixin RtcDualStreamInterface {
  ...
  /// Param [uid] 远端用户的 ID。
  ///
  /// Param [streamType] 设置视频流大小类型。详见 [VideoStreamType]。
  Future<void> setRemoteVideoStreamType(int uid, VideoStreamType streamType);
...
mixin RtcFallbackInterface {
  ...
  /// Param [uid] 远端用户的 ID。
  ///
  /// Param [userPriority] 远端用户的需求优先级。详见 [UserPriority]。
  Future<void> setRemoteUserPriority(int uid, UserPriority userPriority);
}
...
mixin RtcEncryptionInterface {
  ...
  /// **Note**
  /// - 请勿在转码推流场景中使用该方法。
  /// - 该方法需要在 [RtcEngine.setEncryptionSecret] 之后调用。
  ///
...
mixin RtcInjectStreamInterface {
  ...
  ///
  /// Param [config] [LiveInjectStreamConfig] 类，外部输入的音视频流的配置。
  Future<void> addInjectStreamUrl(String url, LiveInjectStreamConfig config);
...
mixin RtcStreamMessageInterface {
  ...
  ///
  /// Param [streamId] [RtcChannel.createDataStream] 方法返回的数据流 ID。
  ///
